  ...
  describe('Belongs To Association', function() {
    ...
    describe('.find()', function() {
      ...
      it('should return customer when the populate criteria is added', function(done) {
        ...
        .populate('customer', {sort: {'name':'asc'}})
        .sort({'amount':'asc'})
        .exec(function(err, _payments) {
          ...
          if (customers[0].name === 'foo') {
            ...
            assert(_payments[0].customer.name === 'bar',
            'Expected `payments[0].customer.name`==="bar", instead payments[0].customer ===> '+ require('util').inspect(_payments[0].customer, false, null));

            assert(_payments[1].customer.name === 'foo',
            'Expected `payments[1].customer.name` === "foo", instead payments[1].customer ===> '+ require('util').inspect(_payments[1].customer, false, null)
            );
          }

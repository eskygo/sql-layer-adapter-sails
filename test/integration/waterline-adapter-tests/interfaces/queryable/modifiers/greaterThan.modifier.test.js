    ...
    describe('greaterThan (>)', function() {
      describe('integers', function() {
        ...
        it('should return records with greaterThan key', function(done) {
          Queryable.User.find({ first_name: testName, age: { greaterThan: 40 }}).sort({'age':'asc'}).exec(function(err, users) {
            assert(!err);
        ...
        it('should return records with symbolic usage > usage', function(done) {
          Queryable.User.find({ first_name: testName, age: { '>': 40 }}).sort({'age':'asc'}).exec(function(err, users) {
            assert(!err);
            assert(Array.isArray(users));
            assert(users.length === 3);
            assert(users[0].age === 41);
    ...
    describe('greaterThanOrEqual (>=)', function() {
      ...
      describe('integers', function() {
        ...
        it('should return records with greaterThanOrEqual key', function(done) {
          Queryable.User.find({ first_name: testName, age: { greaterThanOrEqual: 41 }}).sort({'age':'asc'}).exec(function(err, users) {
            assert(!err);
            assert(Array.isArray(users));
            assert(users.length === 3);
            assert(users[0].age === 41);
        ...
        it('should return records with symbolic usage >= usage', function(done) {
          Queryable.User.find({ first_name: testName, age: { '>=': 41 }}).sort({'age':'asc'}).exec(function(err, users) {
            assert(!err);
            assert(Array.isArray(users));
            assert(users.length === 3);
            assert(users[0].age === 41);

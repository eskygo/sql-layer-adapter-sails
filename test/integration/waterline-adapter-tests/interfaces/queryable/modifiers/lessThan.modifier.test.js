    ...
    describe('integers', function() {
      ...
      describe('lessThan (<)', function() {
        ...
        it('should return records with lessThan key', function(done) {
          Queryable.User.find({ first_name: testName, age: { lessThan: 42 }}).sort({'age':'asc'}).exec(function(err, users) {
            assert(!err);
        ...
        it('should return records with symbolic usage < usage', function(done) {
          Queryable.User.find({ first_name: testName, age: { '<': 42 }}).sort({'age':'asc'}).exec(function(err, users) {
            assert(!err);
            assert(Array.isArray(users));
            assert(users.length === 2);
            assert(users[0].age === 40);
    ...
    describe('lessThanOrEqual (<=)', function() {
      ...
      describe('integers', function() {
        ...
        it('should return records with lessThanOrEqual key', function(done) {
          Queryable.User.find({ first_name: testName, age: { lessThanOrEqual: 42 }}).sort({'age':'asc'}).exec(function(err, users) {
            assert(!err);
            assert(Array.isArray(users));
